...
public class Chart3dDemo extends Application {
    ...
    private float[][] createNoise( int size) {
        ...
        double [][] image = aux.getMatrix();
        int []pixel;
        BufferedImage matrizI = aux.getImage().getBufferedImage();

        noiseArray = new float[image.length][image[0].length];
        for(int i=0;i<image.length;i++)
            for(int j=0;j<image[0].length;j++)
               {
                pixel = matrizI.getRaster().getPixel(i, j, new int[3]);
                //valor = image[i][j];


                double noise = ImprovedNoise.noise(pixel[1], pixel[2], 0);
                noiseArray[i][j] = (float) noise;
        }
